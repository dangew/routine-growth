...
public class RoutineService {
    ...
    public RoutineResponse createRoutine(Long userId, RoutineRequest routineRequest)
        ...
        // Create routine
        Routine routine = Routine.builder().content(routineRequest.getContent()).user(user)
            .category(routineCategory).build();

        // Save routine
        Routine routineSaved = routineRepository.save(routine);

        // Return response
        return RoutineResponse.builder().content(routineRequest.getContent())
            .categoryName(routineSaved.getCategory().getName()).build();
    }
}
